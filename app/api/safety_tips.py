...
async def get_tips_by_category(category_id: int):
...
async def create_tip(tip_data: TipCreate):
    ...
    try:
        with get_db_cursor() as cur:
            ...
            new_tip_row = cur.fetchone()
            new_tip = dict(new_tip_row) if new_tip_row else None

        if not new_tip:
            raise HTTPException(status_code=500, detail="Failed to create safety tip")

        SystemLogsService.create_log(
            action="Safety Tip Created",
            status="Success",
            details=(
                f"Created safety tip id={new_tip['id']} "
                f"(category_id={new_tip['category_id']}, level={new_tip['level']})."
            ),
            user="System Admin",
            role="admin",
        )
...
async def delete_tip_detail(detail_id: int):
    ...
    try:
        with get_db_cursor() as cur:
            cur.execute(
                "DELETE FROM safety_tip_details WHERE id = %s RETURNING id, tip_id",
                (detail_id,),
            )
            deleted = cur.fetchone()
